...
Hence, the answer to the query is 0.</description>
...
class Solution {
    public int[] maximumBeauty(int[][] items, int[] queries) {
        ...
        // Convert beauty into prefix maximum beauty
        // items[i][1] = max beauty among all items with price <= items[i][0]
        ...
        for (int i = 0; i < queries.length; i++) {
            ans[i] = findMaxBeauty(items, queries[i]);
        }

        return ans;
    }

    private int findMaxBeauty(int[][] items, int maxPrice) {
        int left = 0;
        int right = items.length - 1;
        int result = 0;

        // Find the last item whose price <= maxPrice
        while (left <= right) {
