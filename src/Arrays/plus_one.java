...
1 <= digits.length <= 100
0 <= digits[i] <= 9
digits does not contain any leading 0's.</description>
...
            int newValue = reversed[i] + carry;
            carry = newValue > 9 ? newValue / 10 : 0;
            reversed[i] = newValue > 9 ? newValue % 10 : newValue;
        }

        if(carry > 0) {
            int[] temp = new int[reversed.length+1];
            for(int i=0; i<reversed.length;i++) {
                temp[i] = reversed[i];
            }
